...
const translations = {
  en: {
    closed: "Closed",
    // Add other English translations here as needed
  },
  ar: {
    closed: "مغلق",
    // Add other Arabic translations here as needed
  },
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setCountdownDate(new Date("June 15, 2023 00:00:00").getTime())
})
...
function updateCountdown() {
  ...
  if (distance < 0) {
    ...
    // Change competition state if it's available
    const competitionState = document.querySelector(".competition-state")
    if (competitionState && competitionState.classList.contains("available")) {
      competitionState.classList.remove("available")
      competitionState.classList.add("closed")
      competitionState.textContent = translations[currentLanguage].closed || "Closed"
    }
  }
}
...
function initBackgroundAnimation() {
  const mathSymbols = document.querySelectorAll(".bg-object")

  // Position math symbols randomly
